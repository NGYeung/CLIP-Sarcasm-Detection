...
def one_epoch(train_data_loader, model, optimizer, loss_fn, device):
    ...
    for i, (images, input_ids, attention_mask, labels) in enumerate(train_data_loader):
        ...
        if i%5 == 0: print("train_loss = ",b_loss.item())
...
def classification(model, loss_fn, device, batch_size, ratio = 0.02, test_img_dir="Datasets/Images_test/",test_text_path="Datasets/TEXT_test_sentences.csv"):
    ...
    with torch.no_grad():
        ...
        for images, input_ids, attention_mask in test_loader:
            ...
            # calculate acc per minibatch
            _,logits_V = loss_fn.logits(img_features, text_features)
            
    ###Acc and Loss
    epoch_loss = np.mean(epoch_loss)
    return epoch_loss, logits_V 
...
def train(batch_size, epochs, ratio = 0.5, load = 0):
    ...
    loss_fn = I_T_ContrastiveLoss(temperature=1) # change temperature if needed
...
def prediction(model, image_featurs, text_features, query, n=9):
